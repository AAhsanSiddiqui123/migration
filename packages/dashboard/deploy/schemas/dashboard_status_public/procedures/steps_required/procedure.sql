-- Deploy: schemas/dashboard_status_public/procedures/steps_required/procedure to pg
...
CREATE FUNCTION "dashboard_status_public".steps_required (
    vlevel text,
    vrole_id uuid DEFAULT jwt_public.current_user_id()
)
  RETURNS SETOF "dashboard_status_public".app_level_requirements
AS $CODEZ$
BEGIN
  RETURN QUERY
  SELECT 
