...
DECLARE
  v_notes TEXT; 
BEGIN
    SELECT json_agg(notes)::text as notes FROM dashboard_public.user_action_item_verifications where user_id=NEW.user_id AND action_id=NEW.action_id AND user_action_id=NEW.user_action_id AND rejected=true
    INTO v_notes;
