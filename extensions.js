...
const getExtensionInfo = async (packageDir) => {
  if (!packageDir) {
...
const getExtensionName = async (packageDir) => {
  if (!packageDir) {
    packageDir = await path();
  }
...
const writeExtensionMakefile = async ({ path, extname, version }) => {
  writeFileSync(
    path,
    `EXTENSION = ${extname}
...
const writeExtensionControlFile = async ({
  path,
  extname,
  extensions,
  version
...
const writeExtensions = async (extensions) => {
...
module.exports = { getAvailableExtensions, getExtensionInfo, getExtensionName, getInstalledExtensions, writeExtensionMakefile, writeExtensionControlFile, writeExtensions }